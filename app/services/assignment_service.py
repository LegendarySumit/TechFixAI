...
class AssignmentService:
    ...
    async def assign_ticket(
        ...
        if ticket.category and "backend" in ticket.category.lower() or \
           (ticket.technical_area and "backend" in ticket.technical_area.lower()):
            if ticket.priority.value in ["high", "critical"]:
                backend_devs = [
                ...
                if backend_devs:
                    ...
                    assignment_reason = f"Rule: Backend/Infrastructure + {ticket.priority.value.upper()} priority → Backend Team"
        
        # Rule 2: Frontend/UI
        if not best_dev and (ticket.category and "frontend" in ticket.category.lower() or \
           (ticket.technical_area and "frontend" in ticket.technical_area.lower())):
            ...
            if frontend_devs:
                ...
                assignment_reason = f"Rule: Frontend/UI issue → Frontend Team"
        
        # Rule 3: Database + High/Critical
        if not best_dev and (ticket.category and "database" in ticket.category.lower() or \
           (ticket.technical_area and "database" in ticket.technical_area.lower())):
            if ticket.priority.value in ["high", "critical"]:
                ...
                if backend_devs:
                    best_dev = min(backend_devs, key=lambda d: dev_workload[d.id])
                    assignment_reason = f"Rule: Database + {ticket.priority.value.upper()} priority → Backend Team"
